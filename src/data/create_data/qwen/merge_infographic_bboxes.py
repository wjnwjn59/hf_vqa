...
def get_random_font(font_idx: Dict, lang_prefix: str = 'en') -> str:
...
def merge_infographic_data(
    ...
    for wiki_idx, infographic in enumerate(infographic_generated):
        ...
        # Limit to 5-8 layers (excluding base)
        layers = gen_info['layers_all'][:8]
        ...
        # Determine language prefix from bbox index
        lang_prefix = determine_language_prefix(str(selected_bbox_index))
        
        # Select colors and font for this infographic
        num_colors = random.randint(1, 4)
        selected_colors = get_random_colors(color_idx, num_colors)
        selected_font = get_random_font(font_idx, lang_prefix)
