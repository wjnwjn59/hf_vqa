"""Janus Pro Model Inference Module"""
...
_model: Optional[MultiModalityCausalLM] = None
_vl_chat_processor: Optional[VLChatProcessor] = None
...
def get_model(model_dir: str = "/mnt/dataset1/pretrained_fm/deepseek-ai_Janus-Pro-1B") -> Tuple[MultiModalityCausalLM, VLChatProcessor, Any]:
    """Initialize and return the Janus Pro model, processor, and tokenizer."""
    ...
    if _model is None or _vl_chat_processor is None or _tokenizer is None:
        ...
        _vl_chat_processor = VLChatProcessor.from_pretrained(model_dir)
...
def create_conversation(question: str, image_path: str) -> List[Dict[str, Any]]:
    ...
    Format follows Janus structure with special role tags.
    """
    # Load image
...
def inference(questions: List[str], image_paths: List[str], config: Dict[str, Any]) -> List[str]:
    """
    Run inference on Janus Pro model.
    
    Note: Janus Pro processes samples one at a time based on the provided example code.
